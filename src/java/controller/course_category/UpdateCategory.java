...
@MultipartConfig(
        ...
        maxFileSize = 1024 * 1024 * 5, // Kích thước tệp lớn nhất được phép tải lên (5MB)
        maxRequestSize = 1024 * 1024 * 10 // Tổng kích thước tệp tối đa được phép gửi đến máy chủ (10MB)
)
public class UpdateCategory extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (filePart.getSize() != 0) {
            ...
            cdao.updateCategory(new Category(id, name, "", SlugifyUtil.slugify(name, id), dirr + "/" + fileName, description));

        } else {
            cdao.updateCategory(new Category(id, name, "", SlugifyUtil.slugify(name, id), oldCategory.getImg(), description));
        }
