                                ...
                                <div class="table-responsive">  
                                    <table class="table bg-white table-bordered">
                                        <div class="card-header py-3 d-flex flex-column flex-md-row justify-content-between align-items-center">
                                            <h6 class="m-0 font-weight-bold text-primary">News Type Management</h6>
                                            <div style="display: flex; flex-direction: row; align-items: center;">
                                                <form action="newsType" method="get">
                                                ...
                                                <a href="createNewsType" type="button" class="btn btn-primary ml-4" style=" white-space: nowrap;">Create new Type</a>
                                            </div>
                                        ...
                                        <tbody>
                                            <c:if test="${requestScope.news.size()!= 0}">
                                                <c:forEach begin="${0}" end="${requestScope.news.size() - 1}" var="i" step="${1}">
                                                    <tr>
                                                        <td>${requestScope.news[i].name}</td>
                                                        <td style="width: 10%;">
                                                            <a href="updateNewsType?id=${requestScope.news[i].id}" class="btn btn-primary btn-sm btn-circle" style="text-decoration: none; margin-right: 5px" title="Update Type">
                                                                <i class="fa-solid fa-pen-to-square"></i>                                                
                                                            </a>
                                                            <a href="deleteNewsType?id=${requestScope.news[i].id}" class="btn btn-primary btn-sm btn-circle" style="text-decoration: none;" 
                                                    ...
                                                    <c:if test="${i== page}">
                                                        <li class="page-item active" aria-current="page">
                                                            <c:if test="${param.search== null}">
                                                                <a class="page-link"
                                                                   href="newsType?page=${i}">${i}</a>
                                                            </c:if>
                                                            <c:if test="${param.search!= null}">
                                                                <a class="page-link"
                                                                   href="newsType?search=${param.search}&page=${i}">${i}</a>
                                                            </c:if>
                                                        </li>
                                                    </c:if>
                                                    <c:if test="${i!= page}">
