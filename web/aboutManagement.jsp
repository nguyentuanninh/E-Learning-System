                                    ...
                                    <table class="table bg-white table-bordered">
                                        <div
                                            ...
                                            <div
                                                ...
                                                <form action="aboutManagement" method="get">
                                                ...
                                                <a href="createAbout" type="button"
                                                   class="btn btn-primary ml-4"
                                                   style=" white-space: nowrap;">Create About</a>
                                            </div>
                                        ...
                                        <tbody>
                                            <c:if test="${requestScope.ab.size()!= 0}">
                                                <c:forEach begin="${0}"
                                                           end="${requestScope.ab.size() - 1}" var="i"
                                                           step="${1}">
                                                    <tr>
                                                        <td>${requestScope.ab[i].title}</td>
                                                        <td>${requestScope.ab[i].title_vn}</td>
                                                        <td>
                                                            <fmt:formatDate pattern="dd-MM-yyyy"
                                                                            value="${requestScope.ab[i].createdDate}" />
                                                        </td>

                                                        <c:if
                                                            test="${requestScope.ab[i].modifiedDate!=null}">
                                                            <td>
                                                                <fmt:formatDate pattern="dd-MM-yyyy"
                                                                                value="${requestScope.ab[i].modifiedDate}" />
                                                            </td>
                                                        </c:if>
                                                        <c:if
                                                            test="${requestScope.ab[i].modifiedDate==null}">
                                                            <td>No have modified</td>
                                                        </c:if>
                                                        <td style="width: 15%;">
                                                            <a href="updateAbout?id=${requestScope.ab[i].id}"
                                                               class="btn btn-primary btn-sm btn-circle"
                                                               style="text-decoration: none; margin-right: 5px"
                                                               title="Update About">
                                                                <i class="fa-solid fa-pen-to-square"></i>
                                                            </a>
                                                            <a href="aboutDetails?id=${requestScope.ab[i].id}"
                                                               class="btn btn-primary btn-sm btn-circle"
                                                               style="text-decoration: none; margin-right: 5px"
                                                               title="View About">
                                                                <i class="fa-solid fa-eye"></i>
                                                            </a>
                                                            <a href="deleteAbout?id=${requestScope.ab[i].id}"
                                                               class="btn btn-primary btn-sm btn-circle"
                                                               style="text-decoration: none;"
                                                               title="Detele About"
                                                               onclick="return confirm('Are you sure delete news id: ${requestScope.ab[i].id}?');">
                                                    ...
                                                    <c:if test="${i== page}">
                                                        <li class="page-item active"
                                                            ...
                                                            <c:if test="${param.search== null}">
                                                                <a class="page-link"
                                                                   href="aboutManagement?page=${i}">${i}</a>
                                                            </c:if>
                                                            <c:if test="${param.search!= null}">
                                                                <a class="page-link"
                                                                   href="aboutManagement?search=${param.search}&page=${i}">${i}</a>
                                                            </c:if>
                                                        </li>
                                                    </c:if>
                                                    <c:if test="${i!= page}">
